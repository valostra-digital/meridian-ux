...
export class MXMention extends LitElement {
  static styles = css`
    :host {
      display: block;
  ...
  @state()
  private parsedOptions: MentionOption[] = [];
  ...
  @state()
  private dropdownPosition = { top: 0, left: 0 };
  ...
  private updateDropdownPosition(textarea: HTMLTextAreaElement) {
  ...
  private handleOptionClick(option: MentionOption) {
    this.insertMention(option);
  }

  private insertMention(option: MentionOption) {
