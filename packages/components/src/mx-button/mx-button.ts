...
export class MXButton extends LitElement {
  static styles = css`
    ...
    .mx-btn-dashed:active:not(:disabled):not(.mx-btn-loading) {
      color: var(--mx-color-primary-active, #0958d9);
      border-color: var(--mx-color-primary-active, #0958d9);
    }
  ...
  render() {
    ...
    // Capture boolean values explicitly
    const isDefault = effectiveType === 'default';
    const isPrimary = effectiveType === 'primary';
    const isDashed = effectiveType === 'dashed';
    const isText = effectiveType === 'text';
    const isLink = effectiveType === 'link';
    const isOutlined = effectiveType === 'outlined';
    const isSolid = effectiveType === 'solid';
    const isFilled = effectiveType === 'filled';
    
    const classes: Record<string, boolean> = {};
    classes['mx-btn'] = true;
    classes['mx-btn-default'] = isDefault;
    classes['mx-btn-primary'] = isPrimary;
    classes['mx-btn-dashed'] = isDashed;
    classes['mx-btn-text'] = isText;
    classes['mx-btn-link'] = isLink;
    classes['mx-btn-outlined'] = isOutlined;
    classes['mx-btn-solid'] = isSolid;
    classes['mx-btn-filled'] = isFilled;
    if (this.size !== 'middle') {
      classes[`mx-btn-${this.size}`] = true;
    }
    if (this.shape !== 'default') {
      classes[`mx-btn-${this.shape}`] = true;
    }
    classes['mx-btn-danger'] = this.danger;
    classes['mx-btn-loading'] = this.loading;
    classes['mx-btn-ghost'] = this.ghost;
    classes['mx-btn-icon-only'] = this.iconOnly;
    classes['mx-btn-icon-end'] = this.iconPlacement === 'end';
