...
export class MXTimePicker extends LitElement {
  static styles = css`
    :host {
  ...
  private selectSecond(second: number) {
  ...
  private renderColumn(type: 'hour' | 'minute' | 'second' | 'period') {
    let items: (number | string)[] = [];
    let selectedValue: number | string = 0;
    let step = 1;
        ...
        ${items.map(item => {
          ...
          return html`
            <li 
              ...
              @click=${() => {
                if (type === 'hour') this.selectHour(item as number);
                else if (type === 'minute') this.selectMinute(item as number);
                else if (type === 'second') this.selectSecond(item as number);
                else if (type === 'period') this.selectPeriod(item as 'AM' | 'PM');
              }}
            >
